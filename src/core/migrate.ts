...
`);
  ...
  {
    version: 145,
    name: 'source_event_projection_receipts',
    idempotent: true,
    sql: `
      ...
      UPDATE source_event_receipts
         SET source_key='page:' || source_slug
  ...
  {
    version: 148,
    name: 'upstream_v143_v144_skew_repair',
    // Fork builds used v143-v145 for source-event migrations before upstream
    // assigned v143/v144 to Dream TTL and open loops. A brain on that history
    // would otherwise advance past the upstream DDL without ever applying it.
    idempotent: true,
    sql: `
      ALTER TABLE dream_verdicts ADD COLUMN IF NOT EXISTS expires_at TIMESTAMPTZ;
      UPDATE dream_verdicts
