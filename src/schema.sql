...
CREATE TABLE IF NOT EXISTS source_event_receipts (
  ...
  projection_state  TEXT NOT NULL DEFAULT 'committed' CHECK (projection_state IN ('preparing','pending','committing','committed','aborted')),
  run_id            TEXT,
  prior_revision_id TEXT,
  pending_revision_id TEXT,
  pending_fact_ids  JSONB NOT NULL DEFAULT '[]'::jsonb CHECK (jsonb_typeof(pending_fact_ids) = 'array'),
  artifact_hash     TEXT,
...
BEGIN
  ...
  IF has_bypass THEN
    ...
    ALTER TABLE source_event_receipts ENABLE ROW LEVEL SECURITY;
