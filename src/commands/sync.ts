...
async function performSyncInner(engine: BrainEngine, opts: SyncOpts): Promise<SyncResult> {
  ...
  if (filtered.renamed.length > 0) {
    ...
    for (const { from, to } of filtered.renamed) {
      ...
      if (existsSync(filePath)) {
        const result = await importFile(engine, filePath, to, { noEmbed, source_id: opts.sourceId });
  ...
  if (addsAndMods.length > 0) {
    ...
    async function importOnePath(eng: BrainEngine, path: string): Promise<void> {
      ...
      try {
        const result = await importFile(eng, filePath, path, { noEmbed, source_id: opts.sourceId });
...
async function performFullSync(
  ...
  // Thread source_id so full-sync writes land in the right source
  // (without this, writes silently fall back to schema DEFAULT 'default').
  const result = await runImport(engine, importArgs, { commit: headCommit, source_id: opts.sourceId });
