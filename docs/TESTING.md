...
- `test/source-event-handler.serial.test.ts` - provider identity and strict conversation date attestation, explicit date precedence, native role/tool parsing, literal Markdown shielding, observer primary-session evidence, parse withholding, and raw versus normalized extraction input. These are deterministic fixtures; no provider is called.
- `test/source-event-projector.serial.test.ts` - normalized revision replacement and withholding, plus unsynced pending fences, crashed-projector markers, nonblocking same-source contention, other-source progress, concurrent body drift, and late pending rows excluded from the ordinary fact snapshot. Existing correction/retraction fixtures prove the canonical lifecycle still converges. Real-engine fixtures use injected extraction outcomes rather than paid providers.
- `test/helpers/fact-provenance-contract.ts` - shared PGLite/Postgres contract for retained IDs, source sessions, creation timestamps and typed enrichment; canonical overrides, row swaps, supersession, idempotency, transaction rollback and ambiguous-identity refusal. Called from `test/extract-facts-phase.test.ts` and `test/e2e/facts-fence-reconcile-postgres.test.ts`.
- `test/take-missing-mirror.test.ts` + `test/helpers/take-mirror-contract.ts` - native DB-only import, missing-mirror take append and file reimport preserve body/type/title/custom metadata/tags/facts/timeline and current takes, with source isolation and both existing/absent take fences. The same contract runs on Postgres in `test/e2e/takes-write-ops-postgres.test.ts`; imports disable embedding and native take extraction verifies the fence after reimport.
- `test/e2e/facts-fence-reconcile-postgres.test.ts` — `DATABASE_URL`-gated round-trip for the escape-aware fence parser: renders a `## Facts` fence whose cells carry literal pipes, backslashes (Windows paths), and empty cells via `renderFactsTable`, runs canonical fence reconciliation (`runExtractFacts`) on real Postgres, and asserts every cell survives byte-identically with no column shift.
